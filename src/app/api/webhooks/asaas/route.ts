...
import { PixPaymentWebhook } from "@/app/gateways/asaas.gateway";
...
export async function POST(request: NextRequest) {
  ...
  if (body.event === "PAYMENT_RECEIVED" && body.payment.status === "RECEIVED") {
    // update the payment status to paid
    try {
      const payment = await updatePaymentStatus(body.payment.pixQrCodeId, PaymentStatus.PAID);
